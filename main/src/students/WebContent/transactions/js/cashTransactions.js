...
it.cashtx.plot = function(operation, data) {
    ...
    var customerDetail$ = $('.customerDetail'),
        ...
        _saveHandler = function () {
            ...
            $.post('addCashTransaction.do', {flower_tx: JSON.stringify(flowerTx)}, function(respJSON) {
        ...
        _fetchTransaction = function (flowerTxId, operation) {
            $.post('getCashTransactionDetail.do', {flower_tx_id: flowerTxId}, function (respJSON) {
                var resp = $.parseJSON(respJSON);
            }); 
        },
        ...
        _plot = function (operation, flowerTx) {
            ...
            var cashTrasactionInstance = it.cashtx.newInstance($.extend({}, {operation: operation}, it.cashtx.config));
        },
    ...
    plotConfigs: [
        {
            ...
            customGetDataFromDomCb: function(attrRow$){
...
it.cashtx.newInstance = function (config) {
    ...
    var _getPlotConfigForKey = function (attrKey) {
        var plotConfigForKey;
        $.each(_config.plotConfigs, function(i, plotConfig){
            if (plotConfig.name == attrKey) {
                plotConfigForKey = plotConfig;
            }
        });

        return plotConfigForKey;
    };
    ...
    var _getObjFromDom = function () {
        ...
        _parentElem$.find('.eachAttrRow').each(function(index){
            var attrKey = $(this).data('attrKey'),
                attrVal,
                plotConfig = _getPlotConfigForKey(attrKey);

            if (typeof plotConfig.customGetDataFromDomCb === 'function') {
                attrVal = plotConfig.customGetDataFromDomCb($(this));
            } else {
